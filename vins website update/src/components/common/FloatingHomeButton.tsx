  ...
  return (
    ...
    >
      <motion.button
        ...
        whileTap={{ scale: 0.92 }}
        className="group relative flex items-center gap-1.5 sm:gap-2.5 px-2 py-2 sm:px-4.5 sm:py-3 rounded-full bg-gradient-to-r from-[#0A2540] via-[#0F365E] to-[#0A2540] text-white shadow-xl hover:shadow-[0_10px_25px_-5px_rgba(255,107,0,0.5)] border border-amber-400/40 backdrop-blur-lg transition-all duration-300 cursor-pointer select-none active:brightness-110 max-w-[120px] sm:max-w-full"
      >
        {/* Glow ambient pulse */}
        <span className="absolute -inset-0.5 rounded-full bg-gradient-to-r from-[#FF6B00]/40 to-amber-400/40 blur-sm opacity-70 group-hover:opacity-100 transition-opacity pointer-events-none" />

        {/* Icon Circle */}
        <span className="relative z-10 w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-[#FF6B00] group-hover:bg-amber-400 text-white group-hover:text-[#0A2540] flex items-center justify-center shrink-0 shadow-md transition-all duration-300">
          {isHome && scrolledDown ? (
